...
#Drops a series of unnecessary columns in current applications dataset
...
#Drops a series of unnecessary columns in previous application history dataset
...
full_dataset['TOTAL_INCOME'].value_counts(bins = [0, 25000, 50000, 100000, 150000, 200000, 300000, 400000, 500000, full_dataset['TOTAL_INCOME'].max()], sort = False).plot(kind='bar', color = 'skyblue')
plt.title('Yearly income of applicants - Rupees')
plt.show()

#Boxplot income vs. target
sns.boxplot(x='TARGET', y = 'TOTAL_INCOME', data = full_dataset, color= 'skyblue')
plt.title('Income (Rupees) compared to target variable')
plt.show()
...
plt.show()
plt.title('Cost of goods (Rupees) compared to target variable')
...
plt.title('Credit (Rupees) provided compared to target variable')
plt.show()
...
family.plot(kind='bar', figsize = [12,12], stacked=True)
plt.show()
...
val_counts.plot(kind='bar', figsize = [12,12], stacked=True)
plt.show()
...
#Normalize all non-target features (Target being 1st column)
full_dataset_na_filled = full_dataset.apply(lambda x: x.fillna(x.median()), axis=0)

normalized_full_dataset = StandardScaler().fit_transform(full_dataset_na_filled.drop('TARGET', axis = 'columns').values)


pca = PCA().fit(normalized_full_dataset.data)
